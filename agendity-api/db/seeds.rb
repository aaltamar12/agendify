...
# ============================================================================
# ============================================================================
# BUSINESS GOALS — Studio 70 (Plan Inteligente)
# ============================================================================
puts "\n🎯 Creating business goals for Studio 70..."

[
  { goal_type: "monthly_sales", name: "Meta ventas marzo", target_value: 5_000_000, period: "monthly" },
  { goal_type: "break_even", name: "Punto de equilibrio", target_value: 3_000_000, fixed_costs: 3_000_000, period: "monthly" },
  { goal_type: "daily_average", name: "Promedio diario", target_value: 200_000, period: "monthly" },
].each do |attrs|
  goal = BusinessGoal.find_or_initialize_by(business: studio_70, goal_type: attrs[:goal_type])
  goal.update!(attrs.merge(business: studio_70, active: true))
end
puts "  ✅ #{studio_70.business_goals.count} goals for Studio 70"

# ============================================================================
# EMPLOYEE INVITATION — Barbería Elite (for testing portal empleado)
# ============================================================================
puts "\n📨 Creating employee invitation for Barbería Elite..."

unless emp_andres.user_id.present?
  invitation = EmployeeInvitation.find_or_initialize_by(employee: emp_andres, business: barberia_elite)
  invitation.update!(
    email: emp_andres.email || "andres@barberia-elite.com",
    token: invitation.token || SecureRandom.urlsafe_base64(32),
    expires_at: invitation.expires_at || 72.hours.from_now
  )
  puts "  ✅ Invitation for #{emp_andres.name}: #{ENV.fetch('FRONTEND_URL', 'http://localhost:3000')}/employee/register?token=#{invitation.token}"
end

# ============================================================================
# APPOINTMENTS FOR TODAY — All statuses for testing
# ============================================================================
puts "\n📅 Creating test appointments for today (#{Date.current})..."

today = Date.current
# Skip if today is Sunday (closed)
unless today.wday == 0
  # Confirmed appointment (ready for check-in)
  seed_appointment!(
    employee: emp_carlos,
    appointment_date: today,
    start_time: "14:00",
    business: barberia_elite,
    customer: elite_customers[0],
    service: barberia_elite.services.find_by(name: "Corte clásico") || barberia_elite.services.first,
    end_time: "14:30",
    price: 25_000,
    status: :confirmed
  )
  puts "  ✅ Confirmed appointment for today (ready for check-in)"

  # Payment_sent appointment (for Pagos view)
  appt_ps = seed_appointment!(
    employee: emp_andres,
    appointment_date: today,
    start_time: "15:00",
    business: barberia_elite,
    customer: elite_customers[1],
    service: barberia_elite.services.find_by(name: "Corte degradado (fade)") || barberia_elite.services.first,
    end_time: "15:45",
    price: 30_000,
    status: :payment_sent
  )
  Payment.find_or_create_by!(appointment: appt_ps) do |p|
    p.amount = appt_ps.price
    p.payment_method = :transfer
    p.status = :submitted
  end
  puts "  ✅ Payment_sent appointment for today (visible in Pagos)"
end
...
# ============================================================================
puts "\n" + "=" * 60
puts "🧪 TESTING GUIDE — Features by plan"
puts "=" * 60
puts ""
puts "📋 PLAN BASICO (Salón Bella — ana@salon-bella.com):"
puts "  - Agenda, servicios, empleados, clientes"
puts "  - Reservas online, QR scanner"
puts "  - Portal empleado"
puts "  - Bloqueo dias cerrados"
puts "  - ❌ Sin: cierre caja, creditos, tarifas dinamicas, reportes avanzados"
puts ""
puts "📋 PLAN PROFESIONAL (Barbería Elite — carlos@barberia-elite.com):"
puts "  - Todo lo basico +"
puts "  - Cierre de caja con comisiones"
puts "  - Creditos/cashback (configurable por SuperAdmin)"
puts "  - Tarifas dinamicas MANUALES (activas: -15% Lun-Jue, +20% Vie-Sab)"
puts "  - Reportes avanzados + ganancias netas"
puts "  - Ticket VIP, personalizacion marca"
puts "  - Destacado en Explore"
puts "  - ❌ Sin: metas, reconciliacion, sugerencias IA, verificado"
puts ""
puts "📋 PLAN INTELIGENTE (Studio 70 — laura@studio70.com):"
puts "  - Todo lo profesional +"
puts "  - Metas financieras (3 metas configuradas)"
puts "  - Reconciliacion contable (discrepancias de prueba)"
puts "  - Tarifas dinamicas con SUGERENCIAS IA (3 sugerencias pendientes)"
puts "  - Badge verificado (azul) en Explore"
puts "  - Indicador de consistencia en reportes"
puts ""
puts "🔧 FUNCIONALIDADES TRANSVERSALES (todos los planes):"
puts "  - Multiples servicios por cita"
puts "  - Duracion visible al usuario"
puts "  - Preview de cancelacion con penalizacion"
puts "  - Creditos aplicables en reserva (si tiene balance)"
puts "  - Precios dinamicos visibles al usuario (si negocio tiene tarifas)"
puts "  - Banners publicitarios en proceso de reserva"
puts "  - QR scanner (camara) para check-in"
puts "  - Busqueda de clientes en nueva cita"
puts ""
puts "👤 PORTAL EMPLEADO:"
puts "  - Invitacion: #{ENV.fetch('FRONTEND_URL', 'http://localhost:3000')}/employee/register?token=..."
puts "  - O crear cuenta via invitacion desde dashboard Empleados"
puts ""
puts "🔑 SUPERADMIN (admin@agendity.com):"
puts "  - ActiveAdmin: http://localhost:3001/admin"
puts "  - Impersonar negocios (ver plan + status)"
puts "  - Enviar notificaciones de prueba"
puts "  - Reconciliar cualquier negocio"
puts "  - CRUD de banners, planes, notificaciones configurables"
puts "  - Panel financiero (creditos, cierres, pagos, transacciones)"
puts ""
...
puts "  Dynamic Pricing:#{DynamicPricing.count}"
puts "  Business Goals: #{BusinessGoal.count}"
puts "  Ad Banners:     #{AdBanner.count}"
puts "  Notif Configs:  #{NotificationEventConfig.count}"
puts "  Cash Closes:    #{CashRegisterClose.count}"
puts "  Invitations:    #{EmployeeInvitation.count}"
puts ""
